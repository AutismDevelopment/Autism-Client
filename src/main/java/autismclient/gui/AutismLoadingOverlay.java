...
public class AutismLoadingOverlay extends LoadingOverlay {
    ...
    public static LoadingOverlay create(Minecraft minecraft, ReloadInstance reload,
        ...
        if (PackHideState.isActive() || autismclient.util.AutismLiteVariant.enabled()) {
    ...
    private boolean autism$handOffToVanillaWhileHidden() {
        if (!PackHideState.isActive()) return false;
        if (this.autism$fadeOutStart == -1L) {

            this.autism$minecraft.gui.setOverlay(
                new LoadingOverlay(this.autism$minecraft, this.autism$reload, this.autism$onFinish, false));
        } else {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
        if (autism$handOffToVanillaWhileHidden()) return;
