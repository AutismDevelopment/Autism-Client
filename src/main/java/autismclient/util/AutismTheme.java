...
public final class AutismTheme {
    ...
    public static int recolorImagePixel(int argb, Channel ch, State st) {
        ...
        if (!inSubjectBand(h, ch)) return argb;

        float outS = s * st.sat[ci];
        return (a << 24) | (java.awt.Color.HSBtoRGB(st.hue[ci], outS, v) & 0x00FFFFFF);
    }

    private static boolean inSubjectBand(float h, Channel ch) {
        if (ch == Channel.BACKDROP) return h >= PURPLE_BAND_MIN && h <= PURPLE_BAND_MAX;
        return h <= RED_BAND || h >= 1.0f - RED_BAND;
    }
    ...
    public static int recolorImagePixelTo(int argb, float targetHue, float targetSat) {
        ...
        float h = hsb[0], s = hsb[1], v = hsb[2];
        if (s < IMAGE_NEUTRAL_THRESHOLD) return argb;
        if (h < PURPLE_BAND_MIN || h > PURPLE_BAND_MAX) return argb;

        float outS = clamp01(targetSat * (0.65f + 0.35f * s));
        return (a << 24) | (java.awt.Color.HSBtoRGB(targetHue, outS, v) & 0x00FFFFFF);
    }
}
