...
final class MultiMacroRun {
    ...
    private boolean executeLeaf(MacroAction action, MacroActionType type, MultiMacroHost host, long now) {
        switch (type) {
            ...
            case START_MACRO -> {
                String next = ((autismclient.util.macro.StartMacroAction) action).macroName;
    ...
    private static autismclient.util.macro.MacroTemplate.NumberStyle numberStyle(CaptureValueAction.NumberMode mode) {
        return mode == CaptureValueAction.NumberMode.DROP_CENTS
            ? autismclient.util.macro.MacroTemplate.NumberStyle.WHOLE
            : autismclient.util.macro.MacroTemplate.NumberStyle.AUTO;
    }

    private static String normalizeCapturedNumber(String v, boolean embedded,
                                                  autismclient.util.macro.MacroTemplate.NumberStyle style) {
    ...
    private static boolean applyNumberModifier(CaptureValueAction a, Map<String, String> values) {
        ...
        autismclient.util.macro.MacroTemplate.NumberStyle style = numberStyle(
    ...
    private static java.math.BigDecimal parseCapturedNumber(String raw, boolean embedded,
        ...
        try {
            return new java.math.BigDecimal(embedded
                ? autismclient.util.macro.MacroTemplate.parseCaptureNumber(raw, style)
                : autismclient.util.macro.MacroTemplate.parseCaptureNumberStrict(raw, style));
        } catch (RuntimeException ex) {
            return null;
        }
    }
