...
public class InteractEntityAction implements MacroAction, WaitsForGui, PacketOrdered, RaycastAim {
    ...
    public RaycastAim.Target raycastTarget(Minecraft mc) {
        AutismContainerTarget target = resolveAimTarget();
    ...
    private AutismContainerTarget resolveAimTarget() {
        if (targetMode == TargetMode.LAST_TARGET) return AutismSharedState.get().getLastContainerTarget();
