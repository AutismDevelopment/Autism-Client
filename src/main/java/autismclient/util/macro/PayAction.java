...
public class PayAction implements MacroAction {
    ...
    public CompoundTag toTag() {
        ...
        tag.putBoolean("divideEnabled", divideEnabled);
        tag.putBoolean("probeHidden", probeHidden);
        tag.putString("confirmMacro", confirmMacro == null ? "" : confirmMacro);
    ...
    public void fromTag(CompoundTag tag) {
        ...
        if (tag.contains("divideEnabled")) divideEnabled = tag.getBooleanOr("divideEnabled", false);
        if (tag.contains("probeHidden")) probeHidden = tag.getBooleanOr("probeHidden", true);
        confirmMacro = tag.getStringOr("confirmMacro", "");
        players.clear();
    ...
    public String getDisplayName() {
        ...
        return "Pay " + target + (divideEnabled ? " split " : " ") + amount;
    }
    ...
    public static final int CONFIRM_TIMEOUT_MS = 15_000;

    public static boolean runConfirmMacro(String macroName, java.util.function.BooleanSupplier cancelled) {
        if (macroName == null || macroName.isBlank()) return true;

        if (MacroExecutor.isMacroRunning(macroName)) return true;
        ...
        if (runId < 0L) return true;
        long deadline = System.nanoTime() + CONFIRM_TIMEOUT_MS * 1_000_000L;
