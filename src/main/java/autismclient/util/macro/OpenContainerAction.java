...
public class OpenContainerAction implements MacroAction, WaitsForGui, RaycastAim {
    ...
    public RaycastAim.Target raycastTarget(Minecraft mc) {
        if (targetMode == TargetMode.BLOCK) return RaycastAim.Target.ofBlock(blockPos);
        AutismContainerTarget target = targetMode == TargetMode.LAST_TARGET
    ...
    private AutismContainerTarget firstEntityTarget() {
        for (String ref : effectiveEntityTargets()) {
