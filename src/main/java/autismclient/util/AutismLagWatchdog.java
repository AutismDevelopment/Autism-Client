...
public final class AutismLagWatchdog {
    ...
    private static void sample(Minecraft client) {
        ...
        lastReportMs = now;
        fpsAtTrigger = fps;
        if (!AutismPerf.enabled()) {

            AutismClientAddon.LOG.warn(
                "[AutismLagReport] sustained low fps detected (fps={} baseline={})",
    ...
    private static void emitCensus(Minecraft client) {
        ...
        appendSafe(sb, "hud", () -> {
            ...
            if (!autismclient.util.AutismLiteVariant.enabled()) {
                mmSegment = " mm=" + autismclient.util.mm.MatchmakingManager.get().censusSummary();
            }
            return "enabledElements=" + AutismHudManager.enabledElementCount()
