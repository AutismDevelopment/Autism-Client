...
public class AutismServerInfoOverlay extends AutismOverlayBase {
    ...
    private final TabStripNode tabStrip = new TabStripNode();
    ...
    private final SliderNode probeDelaySlider = new SliderNode();
    private final ProgressBarNode scanProgressBar = new ProgressBarNode();
    ...
    private static final String[] TAB_NAMES = autismclient.util.AutismLiteVariant.enabled()
        ? new String[]{"Info", "Plugins"}
        : new String[]{"Info", "Plugins", "DupeDB"};
    ...
    private final List<PendingSuggestionReply> pendingSuggestionReplies = new ArrayList<>();
    private final List<PendingLiveEvidence> pendingLiveEvidence = new ArrayList<>();
    ...
    private void logOverlayFailureOnce(String stage, Throwable t) {
        if (overlayFailureLogKeys.add(stage)) {
    ...
    private static final long SAVED_PLUGIN_CACHE_MAX_AGE_MS = 60L * 24L * 60L * 60L * 1000L;
    ...
    private static final class TabStripNode extends DirectUiNode {
        ...
        TabStripNode setTabs(String... tabs) {
        ...
        TabStripNode setActiveIndex(int activeIndex) {
            this.activeIndex = Math.max(0, Math.min(Math.max(0, tabs.length - 1), activeIndex));
            return this;
        }

        TabStripNode setOnSelect(IntConsumer onSelect) {
    ...
    private static final class SliderNode extends DirectUiNode {
        ...
        SliderNode setRange(float min, float max) {
            this.min = min;
            this.max = Math.max(min, max);
            return setValue(value);
        }

        SliderNode setStep(float step) {
            this.step = Math.max(0.0f, step);
            return setValue(value);
        }

        SliderNode setValue(float value) {
        ...
        SliderNode setOnChange(Consumer<Float> onChange) {
    ...
    private static final class ProgressBarNode extends DirectUiNode {
        ...
        ProgressBarNode setProgress(float progress) {
    ...
    public void restoreState() {
        ...
        if (activeTab == 0) {
            applyInfoLayout();
        } else if (activeTab == 2 && !autismclient.util.AutismLiteVariant.enabled()) {
            applyRadarLayout();
        } else {
    ...
    private void rebuildUi() {
        ...
        if (activeTab == 2 && !autismclient.util.AutismLiteVariant.enabled()) {
            buildRadarUi();
            return;
        }
    ...
    private boolean loadCachedScan(String address, String contextSignature) {
        ...
        if (cached == null) {
            cached = AutismConfig.getGlobal().getPluginScanByAddress(address);
        }
        ...
        if (cached.plugins != null) {
            ...
            int kept = 0;
            for (String plugin : dedupePluginNames(cached.plugins)) {
                if (kept++ >= MAX_SCAN_ENTRIES) break;
        ...
        pluginCommands.keySet().retainAll(detectedPlugins);
        pluginChannels.keySet().retainAll(detectedPlugins);
        pluginGuis.keySet().retainAll(detectedPlugins);
        pluginCopyCommands.keySet().retainAll(detectedPlugins);
        pluginScrollState.jumpTo(0, 0);
        selectedPlugin = null;
        pluginScanDone = true;
        pluginManualSetupOpen = false;
    ...
    private void cacheCurrentScan() {
        ...
        AutismConfig.getGlobal().putPluginScan(scannedServerAddress, "",
            ...
            lastPluginScanAnswered, lastPluginScanRetried, lastPluginScanFailed, serverName, scannedServerAddress);
    }
    ...
    private void syncScanStateForCurrentServer() {
        ...
        if (!currentAddress.equals(lastRadarServerAddress)) {
            ...
            if (!autismclient.util.AutismLiteVariant.enabled()) AutismDupeRadar.clearServerResults();
        }
    ...
    private boolean mergePluginEntry(Map<String, PluginScanEntry> entries, String explicitKey, String pluginName,
        ...
        PluginScanEntry entry = entries.get(key);
        if (entry == null) {
            if (entries.size() >= MAX_SCAN_ENTRIES) return false;
            entry = new PluginScanEntry();
        ...
        if (commands != null) {
            for (String command : commands) {
                ...
                if (!cleanCommand.isEmpty() && (entry.commands.size() < MAX_ENTRY_COMMANDS || entry.commands.contains(cleanCommand))) {
                ...
                if (!cleanCommand.isEmpty() && isCopyCommandEvidence(evidence)
                    ...
                    && (entry.copyCommands.size() < MAX_ENTRY_COMMANDS || entry.copyCommands.contains(cleanCommand))) {
        ...
        if (channels != null) {
            for (String channel : channels) {
                ...
                if (!cleanChannel.isEmpty() && (entry.channels.size() < MAX_ENTRY_DETAILS || entry.channels.contains(cleanChannel))) {
        ...
        if (guis != null) {
            for (String gui : guis) {
                ...
                if (!cleanGui.isEmpty() && (entry.guis.size() < MAX_ENTRY_DETAILS || entry.guis.contains(cleanGui))) {
    ...
    private boolean mergeCommandEvidence(Map<String, PluginScanEntry> entries, String rawCommand, PluginEvidence source) {
        ...
        trackObservedPluginCommand(token);
        if (token.contains(":")) {
    ...
    private void trackObservedPluginCommand(String token) {
        if (observedPluginCommands.size() < MAX_OBSERVED_PLUGIN_COMMANDS || observedPluginCommands.contains(token)) {
    ...
    public boolean autoProbeOnSpawn() {
        ...
        long cachedAt = AutismConfig.getGlobal().getPluginScanTimestampByAddress(address);
        boolean fresh = cachedAt > 0L && (System.currentTimeMillis() - cachedAt) < SAVED_PLUGIN_CACHE_MAX_AGE_MS;
    ...
    private void cancelActivePluginScan(String reason) {
        ...
        pluginScanInProgress = false;
        pendingSuggestionReplies.clear();
        pendingLiveEvidence.clear();
        suggestionMaintenanceDirty = false;
    }

    private void recordPluginScanError(PluginScanSession session, String stage, Throwable error) {
    ...
    private void finalizePluginScan(PluginScanOutcome requestedOutcome, String issue) {
        ...
        runPluginSuggestionMaintenance(true);
    ...
    public void renderBackgroundProbeBanner(GuiGraphicsExtractor ctx) {
        ...
        try {
            ...
            autismclient.gui.vanillaui.components.ProgressBar.render(
                autismclient.gui.vanillaui.UiContexts.overlay(ctx, textRenderer, -1, -1),
                UiBounds.of(innerX, barY, barW, barH), getPluginScanProgress());
    ...
    public void tickBackground() {
        try {
            runPluginSuggestionMaintenance(false);
            syncScanStateForCurrentServer();
            syncPayloadFingerprintsIntoCurrentScan();
    ...
    public void onCommandSuggestionRequest(net.minecraft.network.protocol.Packet<?> packet) {
        ...
        MC.execute(() -> {
            try {
                ...
                logOverlayFailureOnce("suggestion-request", t);
            }
        });
    }
    ...
    public void onOutgoingCommandPacket(net.minecraft.network.protocol.Packet<?> packet) {
        ...
        MC.execute(() -> {
            try {
                ...
                logOverlayFailureOnce("outgoing-command", t);
            }
        });
    }
    ...
    public void onOpenScreenPacket(net.minecraft.network.protocol.Packet<?> packet) {
        ...
        MC.execute(() -> {
            try {
                ...
                logOverlayFailureOnce("open-screen", t);
            }
        });
    }
    ...
    private void mergeLiveCommandEvidence(String rawCommand, PluginEvidence evidence, String guiTitle) {
        ...
        MC.execute(() -> {
            try {
                if (MC.getConnection() == null) return;
                syncScanStateForCurrentServer();
                String root = normalizeCommandToken(rawCommand);

                if (!isServerKnownCommand(root)) return;
                registerRecentCommand(root, rawCommand);
                pendingLiveEvidence.add(new PendingLiveEvidence(rawCommand, evidence, guiTitle));
                suggestionMaintenanceDirty = true;
                runPluginSuggestionMaintenance(false);
            } catch (Throwable t) {
                logOverlayFailureOnce("live-command", t);
            }
        });
    }

    private boolean isServerKnownCommand(String rootToken) {
        if (rootToken == null || rootToken.isBlank()) return false;
        var connection = MC.getConnection();
        var dispatcher = connection == null ? null : connection.getCommands();
        com.mojang.brigadier.tree.RootCommandNode<?> treeRoot = dispatcher == null ? null : dispatcher.getRoot();
        if (treeRoot == null) return false;
        if (treeRoot.getChild(rootToken) != null) return true;
        int colon = rootToken.indexOf(':');
        return colon >= 0 && colon + 1 < rootToken.length()
            && treeRoot.getChild(rootToken.substring(colon + 1)) != null;
    }
    ...
    private boolean attachGuiToCommandEntry(Map<String, PluginScanEntry> entries, String rawCommand, String guiTitle) {
        ...
        if (entry.guis.size() < MAX_ENTRY_DETAILS || entry.guis.contains(cleanTitle)) {
    ...
    public void onCommandSuggestions(int id, net.minecraft.network.protocol.game.ClientboundCommandSuggestionsPacket packet) {
        ...
        MC.execute(() -> {
            ...
            try {
                ...
                if (!tokens.isEmpty()) {
                    pendingSuggestionReplies.add(new PendingSuggestionReply(
    ...
    private static final class PendingLiveEvidence {
        ...
        PendingLiveEvidence(String command, PluginEvidence evidence, String guiTitle) {
    ...
    private static final class PendingSuggestionReply {        final long generation;
        final boolean liveObservation;
        final PluginProbeSpec probe;
        final PluginProbeSpec observed;
        final List<String> tokens;

        PendingSuggestionReply(long generation, boolean liveObservation, PluginProbeSpec probe,
                               PluginProbeSpec observed, List<String> tokens) {
            this.generation = generation;
    ...
    private void runPluginSuggestionMaintenance(boolean force) {
        if (!suggestionMaintenanceDirty) return;
        long now = System.currentTimeMillis();
        if (!force && now - lastSuggestionMaintenanceAt < PLUGIN_SUGGESTION_MAINTENANCE_INTERVAL_MS) return;
        lastSuggestionMaintenanceAt = now;
        suggestionMaintenanceDirty = false;
        try {
            runPluginSuggestionMaintenanceGuarded(force);
        } catch (Throwable t) {
            logOverlayFailureOnce("suggestion-maintenance", t);
        }
    }

    private void runPluginSuggestionMaintenanceGuarded(boolean force) {
        List<PendingSuggestionReply> batch = new ArrayList<>(pendingSuggestionReplies);
        pendingSuggestionReplies.clear();
        List<PendingLiveEvidence> liveBatch = new ArrayList<>(pendingLiveEvidence);
        pendingLiveEvidence.clear();
        if (batch.isEmpty() && liveBatch.isEmpty()) return;
        ...
        for (PendingLiveEvidence live : liveBatch) {
            mergeCommandEvidence(entries, live.command, live.evidence);
            if (live.guiTitle != null && !live.guiTitle.isBlank()) {
                attachGuiToCommandEntry(entries, live.command, live.guiTitle);
            }
        }

        boolean scanData = false;
        for (PendingSuggestionReply reply : batch) {
            if (!reply.liveObservation) {

                PluginScanSession current = activePluginScan;
                if (!pluginScanInProgress || current == null
                    || !current.matches(reply.generation, currentServerAddress())) continue;
                scanData = true;
            }
            for (String text : reply.tokens) {
                processObservedSuggestion(entries, legacyEntries, reply.probe, reply.observed, reply.liveObservation, text);
            }
        }
        ...
        mergeScanEntries(entries, legacyEntries);
        cleanupPluginCommandOwnership(entries);
        if (scanData || pluginScanInProgress) {
            legacyScanEntries.clear();
            legacyScanEntries.putAll(legacyEntries);
            scanWorkingEntries.clear();
            scanWorkingEntries.putAll(entries);
            PluginScanSession session = activePluginScan;
            tracePluginScan(session, "suggestions batch=" + batch.size()
                + " legacy=" + countPluginEntries(legacyScanEntries)
                + " working=" + countPluginEntries(scanWorkingEntries));

            if (!force) updatePluginScanLifecycle();
        } else {
            applyLivePluginEntries(entries);
        }
    }

    private void processObservedSuggestion(Map<String, PluginScanEntry> entries, Map<String, PluginScanEntry> legacyEntries,
                                           PluginProbeSpec probe, PluginProbeSpec observed, boolean liveObservation, String text) {
        ...
        if (text.contains(":")) {
            ...
            mergeCommandEvidence(entries, text, liveObservation ? PluginEvidence.USER_AUTOCOMPLETE : PluginEvidence.COMMAND_TREE);
            return;
        }
    ...
    public void onCommandTreeChanged() {
        ...
        MC.execute(() -> {
            try {
                onCommandTreeChangedGuarded();
            } catch (Throwable t) {
                logOverlayFailureOnce("command-tree", t);
            }
        });
    }
    ...
    @Override public void setCollapsed(boolean c) {
        ...
        if (c) {
            ...
            else if (activeTab == 2 && !autismclient.util.AutismLiteVariant.enabled()) applyRadarLayout();
    ...
    public void render(GuiGraphicsExtractor ctx, int mx, int my, float delta) {
        if (!visible) return;
        try {
            renderGuarded(ctx, mx, my, delta);
        } catch (Throwable t) {
            logOverlayFailureOnce("render", t);
        }
    }
    ...
    private void renderGuarded(GuiGraphicsExtractor ctx, int mx, int my, float delta) {
        ...
        if (nowMs - lastUiRebuildMs >= 100L && (uiRebuildRequested || pluginScanInProgress)
            || activeTab == 2 && !autismclient.util.AutismLiteVariant.enabled() && nowMs - lastUiRebuildMs >= 500L) {
            clickRegions.clear();
            rebuildUi();
        ...
        } else if (!collapsed && activeTab == 2 && !autismclient.util.AutismLiteVariant.enabled() && isRadarAuthenticated()) {
            clickRegions.clear();
                ...
                for (int rowIndex = startIndex; rowIndex < rows.size(); rowIndex++) {
                    ...
                    if (row.header()) {
                        ...
                        if (rowVisible && !standaloneCommandHint) {
                            ...
                            UiRenderer.chevron(ctx, UiBounds.of(clipLeft + 2, rowTextY, 9, theme.fontHeight(UiTone.BODY)), selected, arrowColor);
                        }
    ...
    private void renderRadarRow(GuiGraphicsExtractor ctx, RadarListRow row, int x, int y, int width,
        ...
        if (row.type == RadarRowType.MATCH && row.match != null) {
            ...
            UiRenderer.chevron(ctx, UiBounds.of(x + 2, textY, 9, theme.fontHeight(UiTone.BODY)), expanded, hovered ? theme.color(UiTone.BODY) : theme.color(UiTone.MUTED));
    ...
    private void drawRadarChip(GuiGraphicsExtractor ctx, int x, int y, int width, int height, String label, int color) {
        ...
        autismclient.gui.vanillaui.components.Chip.render(
            autismclient.gui.vanillaui.UiContexts.overlay(ctx, textRenderer, -1, -1),
            UiBounds.of(x, y, width, height), label, color, false, false);
    }
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        if (!visible) return false;
        try {
            return mouseClickedGuarded(mx, my, button);
        } catch (Throwable t) {
            logOverlayFailureOnce("mouseClicked", t);
            return false;
        }
    }
    ...
    private boolean mouseClickedGuarded(double mx, double my, int button) {
        ...
        if (activeTab == 2 && !autismclient.util.AutismLiteVariant.enabled() && isRadarAuthenticated()) {
        ...
        if ((activeTab == 1 && pluginScanDone && !pluginScanInProgress)
                || activeTab == 2 && !autismclient.util.AutismLiteVariant.enabled() && isRadarAuthenticated()) {
    ...
    public boolean mouseScrolled(double mx, double my, double amount) {
        ...
        try {
            return mouseScrolledGuarded(mx, my, amount);
        } catch (Throwable t) {
            logOverlayFailureOnce("mouseScrolled", t);
            return false;
        }
    }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        try {
            ...
            logOverlayFailureOnce("keyPressed", t);
            return false;
        }
    }
    ...
    public boolean charTyped(char chr, int modifiers) {
        try {
            ...
            logOverlayFailureOnce("charTyped", t);
            return false;
        }
    }
