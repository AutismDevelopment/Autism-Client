...
public final class AutismJoinMacroController {
    ...
    private static void runPreJoinActions(java.util.List<autismclient.util.macro.MacroAction> actions) {
        for (autismclient.util.macro.MacroAction macroAction : actions) {
            ...
            if (!prepared.success()) { skipPreJoin(prepared.error()); continue; }
            net.minecraft.client.gui.screens.Screen answered = CustomMenuScreens.openScreen(mc);
            CustomMenuSubmitResult result = CustomMenuAdapterRegistry.submit(snapshot, prepared.submission());
            if (!result.success()) { skipPreJoin(result.error()); continue; }
            for (Packet<?> packet : result.packets()) {

                if (!sendCommonPacket(packet)) { skipPreJoin("Connection closed"); return; }
            }
    ...
    private static void skipPreJoin(String reason) {
        AutismNotifications.warning(reason == null || reason.isBlank() ? "Join custom menu skipped" : reason);
    }
